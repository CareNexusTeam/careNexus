...
public interface AnalyticsRepository extends JpaRepository<ClinicalReport, Long> {
    // Find all clinical reports matching the given report scope (e.g. Department, Doctor, Period)
    List<ClinicalReport> findByScope(String scope);

    // Find clinical reports matching scope case-insensitively
    List<ClinicalReport> findByScopeIgnoreCase(String scope);
}
